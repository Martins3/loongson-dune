...
static void * do_mapping(void *base, unsigned long len)
{
  ...
  // printf("A-----");
    ...
    // 实际上，从来没有分配过 hugetlb 的出来.
    // 对于 __setup_mappings_full 还是可以利用 PERM_BIG 来创建 page.
    // 因为这是 guest page table, 而 guest page table 本身需要两个函数处理.
    //
    // printf("-----B\n");
		mem = mmap((void *) base, len,
			   PROT_READ | PROT_WRITE,
...
static int grow_size(void)
{
	...
	ptr = do_mapping((void *) PAGEBASE + num_pages * PAGE_SIZE,
			 GROW_SIZE * PAGE_SIZE);
	if (!ptr)
		return -ENOMEM;
...
void dune_page_stats(void)
{
  ...
  // TODO dune_printf will implemented later
...
{
	// XXX: Insufficent?
	return (pa >= PAGEBASE) && (pa < PAGEBASE + num_pages*PAGE_SIZE);
}
...
int dune_page_init(void)
{
	...
	mem = do_mapping((void *) PAGEBASE, num_pages * PAGE_SIZE);
  printf("page tables location: %p\n", mem);
	if (!mem)
		return -ENOMEM;

	pages = malloc(sizeof(struct page) * MAX_PAGES);
  printf("struct pages location : %p\n", pages);
	if (!pages)
		goto err;
...
err:
	munmap((void *) PAGEBASE, num_pages * PAGE_SIZE);
	return -ENOMEM;
}
